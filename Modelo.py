...
def sigmoide(t, inflexion):
    return 0.5- (1/(1+m.exp(-t+inflexion)))

def Modelo(tiempo, n, semilla, octavos, umbral_p, limite_hambre, MIN2, MAX2):
    ...
    noise = PerlinNoise(octaves=octavos, seed=semilla) #Creacion del mapa de ruido aleatorio que sirve como mapa de peligros
    ruido = np.zeros((n,n))
    for i in range(n):
        for j in range(n):
    ...
    return Contador_distancia, Contador_interaccion


n_simul = 10 #Numero de simulaciones
cont_dist =0 #Contadores por actualizar
cont_int =0

for i in range(n_simul):
    distancia, inter = Modelo(100, 40, 13, 11, 0, 21, -0.5, 0.5)
    cont_dist += distancia
    cont_int += inter
print( str(cont_dist) + ', ' + str(cont_int))
